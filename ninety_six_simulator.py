...
print(f"Player 1 won {round((p1_wins/desired_games) * 100, 5)}% of the time ({p1_wins} times), while Player 2 won {round((p2_wins/desired_games) * 100, 5)}% of the time ({p2_wins} times)")    
print(f"Ties happened {round((ties/desired_games) * 100, 5)}% of the time ({ties} times)")
print(f"There were {ninety_six} 96 to 0 games (Happened {round((ninety_six/desired_games) * 100)}% of the time)")
